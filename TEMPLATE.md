...
| Service config *(opt)* | `capabilities/<name>/service/` | project | project-local policy/context for a bundled service; `init`/`start`/`run` require explicit project enable, while the engine code runs from the installed bundle |
