...
Auth is a login *ceremony*, not a header value. A declared connection identifies the app — `api_id` literally or through the standard cascade, and the app hash by `secret_env` indirection — while a persisted **session file** holds the account login. So a connection resolves the app id/hash and a session *path*; the session *file* itself is produced by `telegram login`, not by any cascade tier. `telegram login --qr` is the same ceremony through Telethon's QR login token: it still needs the app id/hash and still persists the same session file, but the user approves it from an already-authorized Telegram app instead of entering an SMS/app code. Per SHEBANG's stateful-CLI guidance, `help` opens with the startup protocol and `doctor` doubles as the session-health/recovery point (`doctor` exit 2 → `telegram login` or `telegram login --qr`).

## Per-connection sessions; the connection named `default` is un-keyed

A named connection keys its session per id at `<state-dir>/<id>/session`, per the standard, so two accounts never share a login. For backward-compatible login continuity, the explicitly declared connection named **`default`** keeps its session at the un-keyed `$XDG_STATE_HOME/telegram/session` (with a `~/.config/telegram/session.session` fallback an existing login already occupies) rather than `<state-dir>/default/session`. `--session` / `TELEGRAM_SESSION` is the one-shot path override for either.

Re-authenticating a login anywhere (another host, a phone sign-in) rotates the account session and can invalidate the prior file. That is a property of the session file alone: it says which file still holds a usable login, not how many processes may run against one. Keying per connection puts distinct accounts in distinct session files, and keys nothing by host. What arbitrates running assistant-service daemons, and over what scope, is stated under *State Layout* in [the assistant service guide](guides/assistant-service.md).
