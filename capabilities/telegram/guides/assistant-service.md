...
- A group member entry may set `"reply_mode": "top_level"` for an automation peer. Final responses, progress, control responses, transcription echoes, and error notices triggered by that sender are then sent as new group messages rather than Telegram replies. This prevents a second bot from mechanically treating every response as another turn; the assistant may still address that peer explicitly in its response text when the channel prompt deliberately hands over the turn.
