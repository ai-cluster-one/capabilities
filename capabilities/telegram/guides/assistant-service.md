...
A Claude worker's `context` is what the worker is launched holding, and it carries exactly two keys. `tools` is a list of built-in tool names; the daemon passes it as `--tools`, the harness flag that keeps only the named built-in tools and removes the rest from the worker's schema, so the list is the whole set the worker runs with. `mcp: false` launches the worker with no MCP servers; absent or `true`, the harness loads whatever its own configuration names. Without the block the harness's own defaults stand. The block governs built-in tools and MCP servers and nothing else: skills, plugins and slash commands are not governed by it, and the Codex worker has no such block. It is declared in settings only, at `defaults.workers.claude` and `defaults.voice_agent.workers.claude`; `/set` does not reach it.
