...
An answered call needs that provider's API key, in the environment variable the selected connection names — `gemini_secret_env` (`GOOGLE_API_KEY` when the entry omits it) or `openai_secret_env` (`OPENAI_API_KEY`) — resolved like every other credential. Its prompts are `voice-agent-<provider>` and `voice-delegation-<provider>`, read as one: the first is how the assistant speaks, the second how it hands work over. `service init` seeds both from the provider's own templates, and carries an existing unsuffixed `voice-agent` onto the default provider's key rather than replacing what the project wrote. Without the key, or without a voice prompt, the daemon logs which one is missing and a caller who also has `call_recording` on is recorded as usual instead.
