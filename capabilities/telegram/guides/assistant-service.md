...
- User/member policy: display `name`/`username`, `role`, `may_address`, `control`, `authority`, `allowed_capabilities` (or `capabilities`), `context`/`context_file`, `context_mode`, `project`, `call_recording`, and `voice_agent`; an `allowed_users` entry additionally accepts `worker_timeout`, which is that person's direct chat's worker window in the same position `allowed_groups.<chat_id>.worker_timeout` holds for a room — above `defaults.worker_timeout`, below the channel's own `/set worker-timeout`; group members additionally accept `kind` and `address_aliases`.
...
- `progress_after` is the minimum gap in seconds between one relayed progress line and the next. A turn's first line is never held: it reaches the chat the moment the worker writes it, however briefly the turn has been working, because the point of that line is that the chat hears the work has started. The clock is stamped each time a line is actually sent, so every line after the first owes the window, and a burst of lines inside one window relays at most one beyond the first. The lines it governs are what a worker writes with the `telegram send` named as its progress command, which during a turn queues into the turn's outbox rather than sending. A line the window holds back is dropped where it was read, never queued behind the window and never batched into a digest. `0` is the throttle turned off and relays every line the moment it arrives; the bound is `0`-`3600`, fractional values are accepted, and the shipped default is `15`. Nothing else is governed by it: the `job_submitted` handoff a worker writes to that same outbox is control flow rather than chat text and is never held, and the voice agent paces its own narration with `voice_agent.progress_interval`.
...
- Worker policy: `model`; Claude also accepts `effort` and `context`; Codex accepts `reasoning_effort` and `service_tier`. Voice defaults accept `worker`, `workers`, `model`, `voice`, `greeting`, `history`, `timezone`, `progress_interval`, `recording_caption`, and `prompt_file`.
...
A Claude worker's `context` is what the worker is launched holding, and it carries exactly two keys, each governing one thing. `tools` is the whole built-in set: the daemon passes the list as `--tools`, and the harness keeps the named built-in tools and removes every other built-in tool from the worker's schema. It says nothing about MCP: the tools an MCP server contributes are not built-in, so they stay in the schema whatever `tools` names, until `mcp: false` launches the worker with no MCP servers at all; absent or `true`, the harness loads whatever its own configuration names. A worker meant to hold only what it names declares both. A name in `tools` the harness does not recognise is dropped by the harness without a word, so the set a worker actually holds is read at the harness rather than off the settings. Without the block the harness's own defaults stand. The block governs built-in tools and MCP servers and nothing else: skills, plugins and slash commands are not governed by it, and the Codex worker has no such block. It is declared in settings only, at `defaults.workers.claude` and `defaults.voice_agent.workers.claude`; `/set` does not reach it.
...
- `telegram service reload` applies policy and worker/voice defaults without disconnecting an active call. Prompt files are already read per request or call; reload is for `settings.json`. It follows the account-global owner record, so it runs from any project and reports as `project` the root the reloaded daemon actually runs from; only a reload issued from that same project also refreshes the project-layout snapshot. Without a live recorded owner it refuses, naming the project that owns the account instead of suggesting `service init`, because a second service envelope on one account is the state to avoid.
...
`provider` picks the speech stack the call runs on, and each one answers for its own model, voice, credential and prompts:

- `gemini` (the default) runs Gemini Live, and the model itself both speaks and reasons. Work reaches the project's worker through the `agent_task` tool, alongside the other tools a call may declare.
- `gptlive` runs OpenAI GPT Live as a voice only. Its delegation is client-side, which the protocol gives no tool channel at all: the model hands the turn over and the project's worker is the whole backend, so `tools` is not read and the call carries the conversation to the worker instead of a tool call. Voice and model names belong to this stack's own vocabulary; one written for the other provider is replaced rather than sent, because the service ends a session on a name it does not know.

An answered call needs that provider's API key, in the environment variable the selected connection names — `gemini_secret_env` (`GOOGLE_API_KEY` when the entry omits it) or `openai_secret_env` (`OPENAI_API_KEY`) — resolved like every other credential. Its documents are named for who they are addressed to, the provider first: `<provider>-voice-agent` is written for the model that speaks, and `<provider>-backend` for the worker behind it, reaching it on calls only while `context.md` reaches it everywhere. `service init` seeds the ones that provider has — `gemini-voice-agent` alone for Gemini, whose worker has always run on `context.md`, and both for GPT Live — and carries an existing unsuffixed `voice-agent` onto the default provider's key rather than replacing what the project wrote.

How a hand-off works is not among them. It ships inside the capability, is read from there, and is seeded nowhere: it states what the connection can and cannot do, so a project's edit of it could only be wrong. `<provider>-delegation` is room to add to it — write one and it is read after that prose, never instead of it. A project that names `prompt_file` bypasses all of this and gets exactly the file it named. Without the key, or without a voice prompt, the daemon logs which one is missing and a caller who also has `call_recording` on is recorded as usual instead.
...
The register is the one exception, and for the same reason replies are. A job row belongs to the queue that drains it, which is the daemon's, while the project the work happens in is read back from the channel when the runner claims it. So the worker shim puts the daemon's own project in front of a `jobs` call and no other, and a job registered from a routed turn lands where the runner looks for it rather than in a partition nobody drains. That project carries its gate with it: a routed worker's `jobs` call is governed by the daemon's project policy, because the register it operates is the daemon's, so `telegram` has to be enabled where the daemon runs and disabling it in a routed target leaves the queue reachable.

Routing changes which rights the worker has, not only where it runs. Beyond the register, the target project's capability gate governs the routed worker, so a capability must be enabled there — `capabilities enable <name> --project` inside that project, or `--global` for every project — before the worker can call it. Replies and progress are unaffected: `telegram send` from a worker is intercepted by the worker shim and delivered by the daemon's own client, so it never depends on the worker's directory or on the target project's policy.
