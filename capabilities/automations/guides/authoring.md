...
Give an entry an optional `name` and `description`. Neither changes what runs: they carry what the automation is called and why it exists into `automations list`, so reading the set does not mean opening every script. Every other key is refused, because a key the runtime does not read is a declaration that was silently doing nothing.
