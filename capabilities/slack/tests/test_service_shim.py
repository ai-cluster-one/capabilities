...
def test_post_appends_to_outbox(tmp_path):
    ...
    env = {
        **os.environ,
        "SLACK_WORKER_OUTBOX": str(outbox),
        "SLACK_WORKER_CONVERSATION": "D1",
    }
    r = _run(["post", "current", "hello", "world"], env)
...
def test_read_fails_closed_even_if_real_slack_is_provided(tmp_path):
    env = {
        **os.environ,
        "SLACK_WORKER_OUTBOX": str(tmp_path / "o.jsonl"),
        "SLACK_WORKER_CONVERSATION": "D1",
        "SLACK_REAL_SLACK": "/should/not/run",
    }
    r = _run(["read", "D1"], env)
    assert r.returncode == 4
