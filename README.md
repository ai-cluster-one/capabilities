...
- **Package your own** → `capabilities guide` lists the manager's authoring, contract, development, review, and publishing guides with previews. `capabilities new <name> --source <id>` creates and contract-stamps a canonical source bundle. Local-path install remains available for development, and every installation passes full audit in staging before atomically replacing the registry copy.
