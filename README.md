...
| [YouTrack](capabilities/youtrack/) | drive YouTrack issues and knowledge-base articles over the REST API (discover projects, search/create/read issues, read/add comments, update workflow state, read/create/update articles, read a project's custom-field schema); stable project entity ids fit the project identifiers envelope |
...
| [macmail](capabilities/macmail/) | read and draft mail across Mail.app's configured accounts over macOS Automation (read/search/show/links/attachments/draft/export; never sends) |
...
| [Resend](capabilities/resend/) | send-only email over the Resend HTTP API (resend.com) — deliver mail from a verified sending domain (`send`, write-gated, optional per-connection `allow_to` recipient allow-list), list sending domains + verification status, `doctor` proves key + reachability; one-way by construction — no read or receive surface |
...
| [Slack](capabilities/slack/) | read and post through a bot-token CLI with fail-closed conversation policy, plus an optional Socket Mode assistant service with explicit ingress, write, workspace, and per-role capability gates |
...
| [Cal.com](capabilities/calcomc/) | drive a Cal.com account through the official `calcom` CLI — a credential harness that injects the selected connection's API key, pins the API base, and forwards every command (agenda, bookings, slots, event types, schedules, teams, …) straight to `calcom` with I/O untouched; `doctor` proves the key and reports the account it resolves to; writes are refused by default — a booking reaches real attendees — and granted per connection; requires a resolved key and refuses this machine's stored `calcom login` session |
...
| [callvatest](capabilities/callvatest/) | regression-test a deployed CallVA voice agent against production behaviour — join a real LiveKit room as a headless participant, drive the conversation over text, then assert what the agent did: the transcript, which tools it called with which arguments, the results it answered from, and the fields the platform stored once the call settled; structure is checked deterministically in code while a prose claim is settled by a declared judge model (`fixtures` judges the judge against frozen cases), `probe` places one diagnostic call with no assertions; every run is a real billable call, so `run`/`suite`/`probe` are write-gated |
...
| [Journal](capabilities/journal/) | read and enrich a voice-journal store in the consumer's own PostgreSQL over libpq — list/filter entries, read one in full, write enrichment back (summary, type, language, tags, metadata), chunk a transcript for embedding, three-level search (metadata / summary / full transcript), and an idempotent `schema` bootstrap; entries arrive from the consumer's own capture pipeline, and summaries/tags are generated by the host agent (`journal guide enrichment`); a connection binds one database, optionally scoped to one author, with the password by env-key indirection |
...
- **Install one** → `capabilities install asana` fetches the capability into the registry, symlinks its executable onto `PATH`, snapshots its declaration, and scaffolds credentials per its declared scope. Source-directory installs, including script paths inside a bundle such as `capabilities/telegram/bin/telegram`, copy the bundle assets beside one canonical executable. In a consuming project, `capabilities init --codex` wires `.codex/config.toml`, `.codex/hooks.json`, and manager-owned `.codex/generated/capabilities.md`; `capabilities init --claude`, or bare `capabilities init`, wires Claude Code. When `.contextkit/config.toml` is present, ContextKit owns those host bindings and the combined context, asks the manager for `capabilities context --fragment`, and inserts that manager-owned block verbatim. Capabilities init and the host-writing context modes deliberately skip their own hooks and generated files there. `capabilities enable asana` makes it visible.
- **Package your own** → `capabilities guide` lists the manager's authoring, contract, development, review, and publishing guides with previews. `capabilities new <name> --source <id>` creates and contract-stamps a canonical source bundle. Local-path install remains available for development; installation validates bundle structure and declarations, while authoring audit stays an explicit command.
- **Own a catalogue** → `capabilities source init <id> --remote <git-url>` creates the sole editable workspace at `~/.capabilities/sources/<id>`, vendors the canonical contract, and registers it. Create with `capabilities new <name> --source <id>`, generate with `source index`, validate explicitly with `dev check` or `source verify`, and publish with ordinary git. Other machines attach it with `source add`; see [SOURCES.md](SOURCES.md) for the exact paths and lifecycle.
- **Maintain** → `capabilities update` pulls newer versions, `capabilities uninstall` removes cleanly, `capabilities doctor` reconciles the machine, and `capabilities audit <name>` checks a capability against the contract. Review workflows live in `capabilities guide grooming` and `capabilities guide sanitizing`.
...
`capabilities path --json` is the machine-readable source of truth for the current project root, envelope, and provider. Plain projects resolve to `<project>/capabilities`. ContextKit-bound projects must return one absolute path inside the project; an unavailable, failing, empty, relative, malformed, or outside-project answer is a validation error rather than a fallback to a second envelope.
