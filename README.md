...
| **Host portability** | host-neutral by design (Claude Code today) | Claude Code | broad cross-client *(its strength)* |
...
| [Mailbox](capabilities/mailbox/) | IMAP/SMTP adapter for one mailbox — list/show/fetch/flag/move and **send**; profiles in mailbox.json, app-password in .env |
...
| [WhatsApp](capabilities/whatsapp/) | read a WhatsApp account through a self-hosted **WAHA** (WhatsApp HTTP API) bridge — onboard a session via the GOWS phone-number pairing code (no QR), doctor + instance-level health (engine/mode/account), list chats, fetch/search a chat, contact lookup, export a chat's full history to JSON with voice/audio + photos (idempotent), transcribe voice notes via Deepgram, render a markdown log; profile-indirect creds (`whatsapp.json`, like mailbox) with a single-instance env fallback; reads only — sending is mode-gated and planned |
...
These are the alignment signal — the standard the [audit](procedures/audit.md) walks rule by rule. Want a deep dive? Read them, or point your own Claude Code at this repo and ask it to weigh the convention against whatever you use today, honestly.

## Try it

- **Install one** → hand an agent [INSTALL.md](INSTALL.md) and a capability name. It places everything — registry folder, CLI on PATH, stub, project assets, the loader — by reasoning about your machine. The only file a consumer needs to start.
- **Package your own** → the convention works on *your* tools, not just this catalogue. Point an agent at [procedures/package.md](procedures/package.md) with whatever you've got — a working script, a notebook, a pile of API calls, or just a service you keep reaching for. It works out what you have, tells you honestly whether it *should* be a capability (and points you elsewhere if it shouldn't), then builds a conformant one — executable to standard, credentials on the cascade, a `doctor`, the slots filled, validated clean — and installs it into your own project. You end up owning a new capability built from what you already had.
- **Maintain** → [update.md](procedures/update.md) pulls newer versions, [uninstall.md](procedures/uninstall.md) removes cleanly, [audit.md](procedures/audit.md) is the advisory drift check.

No installer binary — every procedure is plain-English instructions an LLM executes, adapting to the environment by reasoning, not by hardcoded branches.
...
capabilities/<capability>/
  ...
  stub.md                     global awareness line → ~/.claude/tools/<name>.md, @-imported
  bin/<name>                  the executable → symlinked onto PATH
  ...
  project/                    → .capabilities/<namespace>/ in a consuming project
    CAPABILITY.md             role in this project + a pointer list
    identifiers.md            non-secret structural ids only
    reference.md              project-specific model (ships as a self-describing scaffold)
    scripts/                  authored sources (optional)
```

A capability *declares* itself host-neutrally in a registry (`~/.capabilities/<name>/`); *surfacing* that declaration into a session is the host's job. The convention is **designed host-neutral** — Claude Code is the first host (via `@`-imported stubs and a `SessionStart` loader); **Codex and other hosts are pending testing**, not yet proven. Swap the injection for a host's own and the registry plus the CLI-on-PATH route stay identical.
